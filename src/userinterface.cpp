...
UserInterface::UserInterface(SDL_Renderer* renderer) {
	healthTexture.Init(renderer, "beigePanel.png");
	healthTexture.SetNineSrc(5, 32, 32);
	healthTexture.SetNineDst(50, 50, 100, 50, 5);
	healthTitleFont.Init(renderer, "font.ttf", 16, "HEALTH:", Vector2D<int>(55, 55), { 211,191,169,255 });
	healthFont.Init(renderer, "font.ttf", 16, "150", Vector2D<int>(82, 75), { 211,191,169,255 });
	healthFont.BoundingBox({ 50, 50, 100, 50 });

	scoreTexture.Init(renderer, "beigePanel.png");
	scoreTexture.SetNineSrc(5, 32, 32);
	scoreTexture.SetNineDst(850, 50, 100, 50, 5);
	scoreTitleFont.Init(renderer, "font.ttf", 16, "SCORE:", Vector2D<int>(861, 55), { 211,191,169,255 });
	scoreFont.Init(renderer, "font.ttf", 16, "0", Vector2D<int>(894, 75), { 211,191,169,255 });
