...
UserInterface::UserInterface(SDL_Renderer* renderer) {
	...
	healthTitleFont.Init(renderer, "test.ttf", 16, "HEALTH:", Vector2D<int>(55, 55), { 255, 0, 255, 255 });
	healthFont.Init(renderer, "test.ttf", 16, "100", Vector2D<int>(82, 75), { 255, 0, 255, 255 });

	scoreTexture.Init(renderer, "greypanel.png");
	scoreTexture.SetNineSrc(5, 32, 32);
	scoreTexture.SetNineDst(850, 50, 100, 50, 5);
	scoreTitleFont.Init(renderer, "test.ttf", 16, "SCORE:", Vector2D<int>(861, 55), { 255, 0, 255, 255 });
	scoreFont.Init(renderer, "test.ttf", 16, "0", Vector2D<int>(894, 75), { 255, 0, 255, 255 });

	ammoTexture.Init(renderer, "greypanel.png");
	ammoTexture.SetNineSrc(5, 32, 32);
	ammoTexture.SetNineDst(850, 500, 100, 50, 5);
	ammoTitleFont.Init(renderer, "test.ttf", 16, "AMMO:", Vector2D<int>(868, 505), { 255, 0, 255, 255 });
